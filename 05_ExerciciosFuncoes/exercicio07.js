...
const PRESTACAO = BODY.appendChild(document.createElement('input'))
const DIAS = BODY.appendChild(document.createElement('input'))
const BOTAO = BODY.appendChild(document.createElement('button'))
const MSG = BODY.appendChild(document.createElement('p'))

PRESTACAO.setAttribute('placeholder', 'Valor Prestação')
DIAS.setAttribute('placeholder', 'Dias em atraso')

var total = 0

BOTAO.innerText = 'OK'
BOTAO.addEventListener('click', e => {
    var prestacao = parseFloat(PRESTACAO.value)
    var dias = parseInt(DIAS.value)

    function valorPagamento(p, d) {
        if (d > 0) {
            let multa = p * (3 / 100)
            let juros = (p * (0.1 / 100)) * d
            p += multa + juros
        }
        return p.toFixed(2)
    }
    ...
    if (prestacao == 0) {
        MSG.innerText = `Total a pagar: R$ ${total}`
        PRESTACAO.disabled = 'disabled'
        DIAS.disabled = 'disabled'
        BOTAO.disabled = 'disabled'
    } else {
        total += parseFloat(valorPagamento(prestacao, dias))
    }
})
